...
IF(IncludeMonetaryShock==1) THEN
	...
	DO it = 2,Ttransition-nendtrans
		equmTRANS(it)%mpshock =equmINITSS%mpshock *&
(1.0-MonetaryShockPers**deltatransvec(it-1)) + equmTRANS(it-1)%mpshock * (MonetaryShockPers**deltatransvec(it-1))
...
IF(IncludeForwardGuideShock==1) THEN
	...
	DO it = it1+1,Ttransition-nendtrans
		equmTRANS(it)%mpshock =equmINITSS%mpshock *&
(1.0-MonetaryShockPers**deltatransvec(it-1)) + equmTRANS(it-1)%mpshock * &
(MonetaryShockPers**deltatransvec(it-1))
