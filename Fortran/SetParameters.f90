...
tolequmss		= 1.0e-4 !1.0e-10
stepequmss		= 0.10
maxiterequmss	= 40
maxiterrho 		= 30
tolrho			= 1.0e-8

toltransition	= 1.0e-3 !1.0e-6
...
ELSE IF (DividendFundLumpSum ==1) THEN
	...
	IF(DistributeProfitsInProportion==0) lb = ((elast-1.0)/elast)*alpha + (1.0/(1.0-&
fundlev))*targetMeanIll*(deprec+rb*fundlev) + (1.0/elast)*(1.0-corptax)
	IF(DistributeProfitsInProportion==1) lb = ((elast-1.0)/elast)*alpha + (1.0/(1.0-&
fundlev))*targetMeanIll*(deprec+rb*fundlev) + (1.0/elast)*(1.0-corptax)*profdistfrac
...
ALLOCATE(u(ngpa,ngpb,ngpy),c(ngpa,ngpb,ngpy),h(ngpa,ngpb,ngpy),d(ngpa,ngpb,ngpy),&
s(ngpa,ngpb,ngpy),bdot(ngpa,ngpb,ngpy))
...
ALLOCATE(gjoint(ngpa,ngpb,ngpy),gamarg(ngpa,ngpy),gbmarg(ngpb,ngpy),gvec(naby),&
gmat(nab,ngpy),gabmarg(ngpa,ngpb),gabcum(ngpa,ngpb))
ALLOCATE(mpc(ngpa,ngpb,ngpy),subeff1ass(ngpa,ngpb,ngpy),subeff2ass(ngpa,ngpb,ngpy),&
wealtheff1ass(ngpa,ngpb,ngpy),wealtheff2ass(ngpa,ngpb,ngpy))
