...
!$OMP PARALLEL DO
DO iaby = 1,naby
	lcvec(abfromab(afromaby(iaby),bfromaby(iaby)),yfromaby(iaby)) = c(afromaby(iaby),&
bfromaby(iaby),yfromaby(iaby))
	ldvec(abfromab(afromaby(iaby),bfromaby(iaby)),yfromaby(iaby)) = d(afromaby(iaby),&
bfromaby(iaby),yfromaby(iaby))
END DO
!$OMP END PARALLEL DO
...
DO it = 1,nsteps
	...
	DO iy = 1,ngpy
		!consumption	
		!consumption	
		lvec = lccumvec(:,iy) + ldelta*lcvec(:,iy) + ldelta*MATMUL(ymarkovoff(iy,:),&
TRANSPOSE(lccumvec(:,:)))
		lccumvec1(:,iy) = AUMF(iy) .umfpack. lvec
		!deposits
		lvec = ldcumvec(:,iy) + ldelta*ldvec(:,iy) + ldelta*MATMUL(ymarkovoff(iy,:),&
TRANSPOSE(ldcumvec(:,:)))
...
!$OMP PARALLEL DO
DO iaby = 1,naby
	ccum1(afromaby(iaby),bfromaby(iaby),yfromaby(iaby)) = lccumvec(abfromab(afromaby(iaby),&
bfromaby(iaby)),yfromaby(iaby))
	dcum1(afromaby(iaby),bfromaby(iaby),yfromaby(iaby)) = ldcumvec(abfromab(afromaby(iaby),&
bfromaby(iaby)),yfromaby(iaby))
END DO
!$OMP END PARALLEL DO
...
!$OMP PARALLEL DO
DO iaby = 1,naby
	ccum2(afromaby(iaby),bfromaby(iaby),yfromaby(iaby)) = lccumvec(abfromab(afromaby(iaby),&
bfromaby(iaby)),yfromaby(iaby))
	dcum2(afromaby(iaby),bfromaby(iaby),yfromaby(iaby)) = ldcumvec(abfromab(afromaby(iaby),&
bfromaby(iaby)),yfromaby(iaby))
END DO
!$OMP END PARALLEL DO
...
!$OMP PARALLEL DO
DO iaby = 1,naby
	ccum4(afromaby(iaby),bfromaby(iaby),yfromaby(iaby)) = lccumvec(abfromab(afromaby(iaby),&
bfromaby(iaby)),yfromaby(iaby))
	dcum4(afromaby(iaby),bfromaby(iaby),yfromaby(iaby)) = ldcumvec(abfromab(afromaby(iaby),&
bfromaby(iaby)),yfromaby(iaby))
END DO
!$OMP END PARALLEL DO
