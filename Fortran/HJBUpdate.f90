...
REAL(8) 		:: lVaF,lVaB,lVbF,lVbB,lcF,lcB,lc0,lsF,lsB,&
ls0,lHcF,lHcB,lHc0,ldFB,ldBF,ldBB,lHdFB,lHdBF,lHdBB,lhF,lhB,lh0,llabdisutil
INTEGER 		:: validF,validB,validFB,validBF,validBB
REAL(8) 		:: ladriftB,ladriftF,lbdriftB,lbdriftF,lval,&
laudriftB,laudriftF,lbudriftB,lbudriftF
...
DO iaby = 1,naby
	...
	IF(DistributeProfitsInProportion==1 .and. TaxHHProfitIncome==0) gbdrift = &
gbdrift + (1.0-profdistfrac)*(1.0-corptax)*profit*ygrid(iy)/meanlabeff
	IF(DistributeProfitsInProportion==1 .and. TaxHHProfitIncome==1) gbdrift = &
gbdrift + (1.0-labtax)*(1.0-profdistfrac)*(1.0-corptax)*profit*ygrid(iy)/meanlabeff
	gill = agrid(ia)
	...
	IF(ia>1 ) THEN !a backward, b backward
 		IF (ib==1) THEN
			IF(ScaleDisutilityIdio==0) llabdisutil = &
lhB**(1.0+1.0/frisch)/(1.0+1.0/frisch)
			IF(ScaleDisutilityIdio==1) llabdisutil = &
ygrid(iy)*lhB**(1.0+1.0/frisch)/(1.0+1.0/frisch)
	...
	IF (validFB==1 .and. (validBF==0 .or. lHdFB>=lHdBF) .and. &
(validBB==0 .or. lHdFB>=lHdBB)) THEN !forward, backward
		d(ia,ib,iy) = ldFB					
	ELSE IF ((validFB==0 .or. lHdBF>=lHdFB) .and. validBF==1 .and. &
(validBB==0 .or. lHdBF>=lHdBB)) THEN !backward,forward
		d(ia,ib,iy) = ldBF
	ELSE IF ((validFB==0 .or. lHdBB>=lHdFB) .and. (validBF==0 .or. &
lHdBB>=lHdBF) .and. validBB==1) THEN !backward,backward
		d(ia,ib,iy) = ldBB	
...
DO iy = 1,ngpy
	...
	DO iab = 1,nab !cannot be done in parallel
		...
		!b drift
		lbdriftB = min(-d(ia,ib,iy) - adjcostfn(d(ia,ib,iy),agrid(ia)),0.0) + &
min(s(ia,ib,iy),0.0_8)
		lbdriftF = max(-d(ia,ib,iy) - adjcostfn(d(ia,ib,iy),agrid(ia)),0.0) + &
max(s(ia,ib,iy),0.0_8)
		...
		!b drift,upwind
		IF(ib<ngpb) lbudriftB = min(s(ia,ib,iy) -d(ia,ib,iy) - &
adjcostfn(d(ia,ib,iy),agrid(ia)),0.0_8)
		IF(ib<ngpb) lbudriftF = max(s(ia,ib,iy) -d(ia,ib,iy) - &
adjcostfn(d(ia,ib,iy),agrid(ia)),0.0_8)
		IF(ib==ngpb) lbudriftB = min(s(ia,ib,iy) -d(ia,ib-1,iy) - &
adjcostfn(d(ia,ib-1,iy),agrid(ia)),0.0_8)
		IF(ib==ngpb) lbudriftF = max(s(ia,ib,iy) -d(ia,ib-1,iy) - &
adjcostfn(d(ia,ib-1,iy),agrid(ia)),0.0_8)
		...
		!diagonal value
		lval = 0.0_8
		IF(ia>1 .and. ia<ngpa) lval = lval + ladriftB/dagrid(ia-1) - &
ladriftF/dagrid(ia)
		IF(ia==1) lval = lval - ladriftF/dagrid(ia)
		IF(ia==ngpa) lval = lval + ladriftB/dagrid(ia-1)
		IF(ib>1 .and. ib<ngpb) lval = lval + lbdriftB/dbgrid(ib-1) - &
lbdriftF/dbgrid(ib)
		...
		lval = 0.0_8
		IF(ia>1 .and. ia<ngpa) lval = lval + laudriftB/dagrid(ia-1) - &
laudriftF/dagrid(ia)
		IF(ia==1) lval = lval - laudriftF/dagrid(ia)
		IF(ia==ngpa) lval = lval + laudriftB/dagrid(ia-1)
		IF(ib>1 .and. ib<ngpb) lval = lval + lbudriftB/dbgrid(ib-1) - &
lbudriftF/dbgrid(ib)
	...
	CALL coocsr (nab, ACOO(iy)%nz, ACOO(iy)%val(1:ACOO(iy)%nz), &
ACOO(iy)%row(1:ACOO(iy)%nz), ACOO(iy)%col(1:ACOO(iy)%nz), ACSR(iy)%val, ACSR(iy)%col, ACSR(iy)%row)
	...
	CALL coocsr (nab, AUCOO(iy)%nz, AUCOO(iy)%val(1:AUCOO(iy)%nz), &
AUCOO(iy)%row(1:AUCOO(iy)%nz), AUCOO(iy)%col(1:AUCOO(iy)%nz), AUCSR(iy)%val, AUCSR(iy)%col, AUCSR(iy)%row)
	...
	CALL apldia (BCSR(iy)%n, 0, BCSR(iy)%val, BCSR(iy)%col, BCSR(iy)%row, ldiag, &
BCSR(iy)%val, BCSR(iy)%col, BCSR(iy)%row, iw )
...
IF (ReportNonMonotonicity==1) THEN
	DO iaby = 1,naby
		IF(bfromaby(iaby)>1) THEN
			IF (Vnew(afromaby(iaby),bfromaby(iaby),yfromaby(iaby))<&
Vnew(afromaby(iaby),bfromaby(iaby)-1,yfromaby(iaby)) -1.0e-8) &
				write(*,*) 'non-monotonicity, b dimension, ib = ',&
bfromaby(iaby)-1,' ia = ',afromaby(iaby)
		END IF
		IF(afromaby(iaby)>1) THEN
			IF (Vnew(afromaby(iaby),bfromaby(iaby),yfromaby(iaby))<&
Vnew(afromaby(iaby)-1,bfromaby(iaby),yfromaby(iaby)) -1.0e-8) &
				write(*,*) 'non-monotonicity, a dimension, ia = ',&
afromaby(iaby)-1,' ib = ',bfromaby(iaby)
		END IF
	END DO
END IF
