...
OPEN(3, FILE = trim(OutputDir) // 'paramguess' //   '.txt', STATUS = 'replace'); &
CALL WriteMatrixLong(3,Nparam,1,paramguess)
OPEN(3, FILE = trim(OutputDir) // 'paramlb' //   '.txt', STATUS = 'replace'); &
CALL WriteMatrixLong(3,Nparam,1,paramlb)
OPEN(3, FILE = trim(OutputDir) // 'paramub' //   '.txt', STATUS = 'replace'); &
CALL WriteMatrixLong(3,Nparam,1,paramub)
...
DO j = 1,ndfls
	...
	IF(CalibrateRhoAtInitialGuess==1) THEN
		...
		converged = .false.
		neqmiter = 1
		OPEN(3, FILE = trim(OutputDir) // 'DiscountRateAtInitialGuess.txt', &
STATUS = 'replace'); CLOSE(3)
		lrhoL = invlogistic(exp(-0.02_8))
		lrhoU = invlogistic(exp(-0.01_8))
		CALL rtflsp(FnDiscountRate,lrhoL,lrhoU,1.0e-8_8,tolrho,iflag,maxiterrho)
		converged = .true.
...
IF(DistributeProfitsInProportion == 1 .and. TaxHHProfitIncome == 1) taxrev = taxrev + &
labtax*(1.0-profdistfrac)*profit*(1.0-corptax)
