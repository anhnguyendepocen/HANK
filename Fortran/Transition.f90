...
DO it = Ttransition,1,-1
	...
	bdrift = MERGE((rb+PerfectAnnuityMarkets*deathrate)*bgrid,&
(rborr+PerfectAnnuityMarkets*deathrate)*bgrid,bgrid>0.0)
	
	
	nblviolated = .false.
	lbgrid(:,it) = bgrid
	IF (Borrowing==1 .and. rborr+PerfectAnnuityMarkets*deathrate>0.0 &
.and. bgrid(1) < -lumptransfer/(rborr+PerfectAnnuityMarkets*deathrate)) THEN
		IF(Display>=1) write(*,*) '  Warning: natural borrowing limit violated in transition'
		IF(Display>=1) write(*,*) '  Steady State ABL: ',abl, 'Current NBL: ',&
-lumptransfer/(rborr+PerfectAnnuityMarkets*deathrate)
		IF(Display>=1) THEN
		...
		CALL PowerSpacedGrid (ngpbNEG/2+1,bgridparamNEG,lbmin,&
(lbmin+lbgrid(ngpbNEG+1,it))/2.0,lbgrid(1:ngpbNEG/2+1,it))
		DO ib = ngpbNEG/2+2,ngpbNEG
			lbgrid(ib,it) = lbgrid(ngpbNEG+1,it) -(lbgrid(ngpbNEG+2-ib,it)-lbgrid(1,it))
		END DO
		
		bdrift = MERGE((rb+PerfectAnnuityMarkets*deathrate)*lbgrid(:,it),&
(rborr+PerfectAnnuityMarkets*deathrate)*lbgrid(:,it),lbgrid(:,it)>0.0)
...
! 	$OMP PARALLEL DO
	DO iy = 1,ngpy
		...
		ALLOCATE(solnTRANS(it)%A(iy)%val(ACSR(iy)%nz),solnTRANS(it)%A(iy)%row(ACSR(iy)%n+1),&
solnTRANS(it)%A(iy)%col(ACSR(iy)%nz))
		...
		IF (ALLOCATED(solnTRANS(it)%AU(iy)%col)) DEALLOCATE(solnTRANS(it)%AU(iy)%col) 
 		ALLOCATE(solnTRANS(it)%AU(iy)%val(AUCSR(iy)%nz),solnTRANS(it)%AU(iy)%row(AUCSR(iy)%n+1),&
solnTRANS(it)%AU(iy)%col(AUCSR(iy)%nz))
...
DO it = 1,Ttransition
	...
	DO iy = 1,ngpy
		...
		ALLOCATE(solnTRANS(it)%B(iy)%val(solnTRANS(it)%B(iy)%nz),solnTRANS(it)%B(iy)%&
row(solnTRANS(it)%B(iy)%n+1),solnTRANS(it)%B(iy)%col(solnTRANS(it)%B(iy)%nz))

		!transpose
		solnTRANS(it)%B(iy) = solnTRANS(it)%AU(iy)
		CALL csrcsc (nab, 1, 1, solnTRANS(it)%AU(iy)%val, solnTRANS(it)%AU(iy)%col, &
solnTRANS(it)%AU(iy)%row, solnTRANS(it)%B(iy)%val, solnTRANS(it)%B(iy)%col, solnTRANS(it)%B(iy)%row )

		!adjust A' matrix for non-linearly spaced grids
		ldiag = 1.0/(ldavec*ldbvec)
		CALL diamua (nab, 1, solnTRANS(it)%B(iy)%val, solnTRANS(it)%B(iy)%col, &
solnTRANS(it)%B(iy)%row, ldiag,solnTRANS(it)%B(iy)%val, solnTRANS(it)%B(iy)%col, solnTRANS(it)%B(iy)%row )
		ldiag = ldavec*ldbvec
		CALL amudia (nab, 1, solnTRANS(it)%B(iy)%val, solnTRANS(it)%B(iy)%col, &
solnTRANS(it)%B(iy)%row, ldiag,solnTRANS(it)%B(iy)%val, solnTRANS(it)%B(iy)%col, solnTRANS(it)%B(iy)%row )
		...
		CALL apldia (nab, 0, solnTRANS(it)%B(iy)%val, solnTRANS(it)%B(iy)%col, &
solnTRANS(it)%B(iy)%row, ldiag, solnTRANS(it)%B(iy)%val, solnTRANS(it)%B(iy)%col, solnTRANS(it)%B(iy)%row, iw )
	...
	IF(it==1 .and. DividendFundLumpSum==1 .and. OneAssetNoCapital==0) THEN
		...
		DO iy = 1,ngpy
			DO ib = 1,ngpb
				CALL AdjustDistProportionately(agrid,adelta,&
solnINITSS%gjoint(:,ib,iy),equmTRANS(1)%illassetdrop,lgjoint(:,ib,iy))
			END DO
		END DO
		!$OMP END PARALLEL DO
	...
	DO iy = 1,ngpy
		...
		lgmat1(:,iy) = lgmat1(:,iy) + deltatransvec(it)*deathrate*&
MERGE(SUM(lgmat(:,iy)*ldavec*ldbvec),0.0_8,afromab==1 .and. bfromab==ngpbNEG+1 )/(ldavec*ldbvec)
	...
	bgrid = holdbgrid
	statsTRANS(it) = DistributionStatsType(Ea,Eb,Ec,Elabor,Ed,Ewage,Enetlabinc,Egrosslabinc,Enetprofinc,&
Egrossprofinc,Einc,Ehours,Enw,FRACa0,FRACa0close,FRACb0,FRACb0close,FRACb0a0,&
FRACb0aP,FRACbN,FRACnw0,FRACnw0close,FRACb0a0close, &
