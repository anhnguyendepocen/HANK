...
DO iy = 1,ngpy
	...
	ALLOCATE(BCSR(iy)%val(AUCSR(iy)%nz), BCSR(iy)%col(AUCSR(iy)%nz), &
BCSR(iy)%row(AUCSR(iy)%n+1))
	
	!transpose AU matrix
	BCSR(iy) = AUCSR(iy)
	CALL csrcsc (nab, 1, 1, AUCSR(iy)%val, AUCSR(iy)%col, AUCSR(iy)%row, &
BCSR(iy)%val, BCSR(iy)%col, BCSR(iy)%row )

	!adjust A' matrix for non-linearly spaced grids
	ldiag = 1.0/(ldavec*ldbvec)
	CALL diamua (nab, 1, BCSR(iy)%val, BCSR(iy)%col, BCSR(iy)%row, ldiag,&
BCSR(iy)%val, BCSR(iy)%col, BCSR(iy)%row )
	ldiag = ldavec*ldbvec
	CALL amudia (nab, 1, BCSR(iy)%val, BCSR(iy)%col, BCSR(iy)%row, ldiag,&
BCSR(iy)%val, BCSR(iy)%col, BCSR(iy)%row )
	...
	CALL apldia (nab, 0, BCSR(iy)%val, BCSR(iy)%col, BCSR(iy)%row, ldiag, &
...
DO WHILE (ldiff>KFEtol .and. it<maxiterKFE)
	...
	DO iy = 1,ngpy	
 		...
 		lgmat = lgmat + deltakfe*deathrate*MERGE(SUM(gmat(:,iy)*ldavec*ldbvec),&
0.0_8,afromab==1 .and. bfromab==ngpbNEG+1 )/(ldavec*ldbvec)
