...
ELSE IF (ReadEarningsProcess==1) THEN
	OPEN(1, FILE = trim(EarningsProcessDir) // &
'/ygrid_combined.txt');READ(1,*) logygrid;CLOSE(1)
	OPEN(1, FILE = trim(EarningsProcessDir) // &
'/ydist_combined.txt');READ(1,*) ydist;CLOSE(1)
	OPEN(1, FILE = trim(EarningsProcessDir) // &
'/ymarkov_combined.txt');READ(1,*) ymarkov;CLOSE(1)
	IF (AdjustProdGridFrisch==1) logygrid = &
logygrid/ (1.0+adjfricshgridfrac*frisch)
	ygrid = exp(logygrid)
...
ELSE IF(Borrowing==1) THEN
	...
	CALL PowerSpacedGrid (ngpbNEG/2+1,bgridparamNEG,abl,&
(abl+bgrid(ngpbNEG+1))/2.0,bgrid(1:ngpbNEG/2+1))
