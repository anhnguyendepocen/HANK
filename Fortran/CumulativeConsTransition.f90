...
DO it = 3,1,-1
	...
	!$OMP PARALLEL DO
	DO iaby = 1,naby
		lcvec(abfromab(afromaby(iaby),bfromaby(iaby)),yfromaby(iaby)) = &
solnTRANS(it)%c(afromaby(iaby),bfromaby(iaby),yfromaby(iaby))
		ldvec(abfromab(afromaby(iaby),bfromaby(iaby)),yfromaby(iaby)) = &
solnTRANS(it)%d(afromaby(iaby),bfromaby(iaby),yfromaby(iaby))
	END DO
	!$OMP END PARALLEL DO
