...
REAL(8), DIMENSION(naby) :: la,lb,lc,ld,lwage,lnetlabinc,lgrosslabinc,ladelta,&
lbdelta,lhours,ladjcost,lgrossinc,lnw,llabor,lgrossprofinc,lnetprofinc
REAL(8)	:: lbmargdist(ngpb),lamargdist(ngpa),lbmargcum(ngpb),lamargcum(ngpa),&
lpvec(11),linterp1,linterp2
INTEGER, DIMENSION(nab)		:: ordernw
REAL(8), DIMENSION(nab)		:: lnwmargdist,lnwmargcum,lnwgrid,lnwdelta,&
lnw_a,lnw_b,lnw_c,lnw_inc,lnw_h
REAL(8), DIMENSION(naby)	:: lcmargdist,lcmargcum,lcgrid,lcdelta,lmargdist
REAL(8), DIMENSION(naby)	:: lincgrid,lincmargdist,lincdelta,lincmargcum,&
linc_a,linc_b,linc_c,linc_nw,linc_h
...
DO iaby = 1,naby
	...
	IF(DistributeProfitsInProportion==1) lgrossprofinc(iaby) = &
(1.0-profdistfrac)*profit*ygrid(iy)/meanlabeff
	...
	lgrossinc(iaby) = lgrosslabinc(iaby) + lgrossprofinc(iaby) + &
(rb+PerfectAnnuityMarkets*deathrate)*max(bgrid(ib),0.0) + &
(rborr+PerfectAnnuityMarkets*deathrate)*min(bgrid(ib),0.0) + &
(ra+PerfectAnnuityMarkets*deathrate)*agrid(ia)
...
IF ((iteratingtransition .eqv. .false.) .and. (calibrating .eqv. .false.) ) THEN
	...
	DO iab = 1,nab
		...
		IF( SUM(lmargdist, MASK = abfromaby == ordernw(iab)) > 1.0e-12) THEN
			lnw_a(iab)  = SUM(la*lmargdist, MASK = abfromaby == ordernw(iab)) &
/  SUM(lmargdist, MASK = abfromaby == ordernw(iab))
			lnw_b(iab)  = SUM(lb*lmargdist, MASK = abfromaby == ordernw(iab)) &
/  SUM(lmargdist, MASK = abfromaby == ordernw(iab))
			lnw_c(iab)  = SUM(lc*lmargdist, MASK = abfromaby == ordernw(iab)) &
/  SUM(lmargdist, MASK = abfromaby == ordernw(iab))
			lnw_h(iab)  = SUM(lhours*lmargdist, MASK = abfromaby == ordernw(iab)) &
/  SUM(lmargdist, MASK = abfromaby == ordernw(iab))
			lnw_inc(iab)  = SUM(lgrossinc*lmargdist, MASK = abfromaby == ordernw(iab)) &
/  SUM(lmargdist, MASK = abfromaby == ordernw(iab))
		ELSE
			lnw_a(iab)  = 0.0
			lnw_b(iab)  = 0.0
...
IF (iteratingtransition .eqv. .false.) THEN !since we don't store gabcum
	CALL BiLinInterp1 (ngpa,agrid,ngpb,bgrid,gabcum,defnaclose*Egrosslabinc,&
defnbclose*Egrosslabinc,linterp2)
...
IF ((iteratingtransition .eqv. .false.) .and. (calibrating .eqv. .false. .or. MatchMedianLiq==1) .and. &
(ngpy>1 .or. deathrate>0.0)) THEN
	DO ip = 1,11
...
IF ((iteratingtransition .eqv. .false.) .and. (calibrating .eqv. .false. .or. MatchMedianIll==1 .or. &
MatchP75Ill==1) .and. (ngpy>1 .or. deathrate>0.0)) THEN
	DO ip = 1,11
...
IF ((iteratingtransition .eqv. .false.) .and. (calibrating .eqv. .false.) .and. (ngpy>1 .or. deathrate>0.0)) THEN
	...
	CALL ConditionalExpectation (nab,lnwgrid,lnwmargdist,lnw_a,lnwdelta,lnwgrid(1),&
PERCnw(5),Ea_nwQ(1))
	CALL ConditionalExpectation (nab,lnwgrid,lnwmargdist,lnw_a,lnwdelta,PERCnw(5),&
PERCnw(6),Ea_nwQ(2))
	CALL ConditionalExpectation (nab,lnwgrid,lnwmargdist,lnw_a,lnwdelta,PERCnw(6),&
PERCnw(7),Ea_nwQ(3))
	CALL ConditionalExpectation (nab,lnwgrid,lnwmargdist,lnw_a,lnwdelta,PERCnw(7),&
lnwgrid(nab),Ea_nwQ(4))

	CALL ConditionalExpectation (nab,lnwgrid,lnwmargdist,lnw_b,lnwdelta,lnwgrid(1),&
PERCnw(5),Eb_nwQ(1))
	CALL ConditionalExpectation (nab,lnwgrid,lnwmargdist,lnw_b,lnwdelta,PERCnw(5),&
PERCnw(6),Eb_nwQ(2))
	CALL ConditionalExpectation (nab,lnwgrid,lnwmargdist,lnw_b,lnwdelta,PERCnw(6),&
PERCnw(7),Eb_nwQ(3))
	CALL ConditionalExpectation (nab,lnwgrid,lnwmargdist,lnw_b,lnwdelta,PERCnw(7),&
lnwgrid(nab),Eb_nwQ(4))

	CALL ConditionalExpectation (nab,lnwgrid,lnwmargdist,lnw_c,lnwdelta,lnwgrid(1),&
PERCnw(5),Ec_nwQ(1))
	CALL ConditionalExpectation (nab,lnwgrid,lnwmargdist,lnw_c,lnwdelta,PERCnw(5),&
PERCnw(6),Ec_nwQ(2))
	CALL ConditionalExpectation (nab,lnwgrid,lnwmargdist,lnw_c,lnwdelta,PERCnw(6),&
PERCnw(7),Ec_nwQ(3))
	CALL ConditionalExpectation (nab,lnwgrid,lnwmargdist,lnw_c,lnwdelta,PERCnw(7),&
lnwgrid(nab),Ec_nwQ(4))

	CALL ConditionalExpectation (nab,lnwgrid,lnwmargdist,lnwgrid,lnwdelta,lnwgrid(1),&
PERCnw(5),Einc_nwQ(1))
	CALL ConditionalExpectation (nab,lnwgrid,lnwmargdist,lnwgrid,lnwdelta,PERCnw(5),&
PERCnw(6),Einc_nwQ(2))
	CALL ConditionalExpectation (nab,lnwgrid,lnwmargdist,lnwgrid,lnwdelta,PERCnw(6),&
PERCnw(7),Einc_nwQ(3))
	CALL ConditionalExpectation (nab,lnwgrid,lnwmargdist,lnwgrid,lnwdelta,PERCnw(7),&
lnwgrid(nab),Einc_nwQ(4))
	
	!statistics conditional on quartile of total gross income distributions
	CALL ConditionalExpectation (naby,lincgrid,lincmargdist,linc_a,lincdelta,&
lincgrid(1),PERCinc(5),Ea_incQ(1))
	CALL ConditionalExpectation (naby,lincgrid,lincmargdist,linc_a,lincdelta,&
PERCinc(5),PERCinc(6),Ea_incQ(2))
	CALL ConditionalExpectation (naby,lincgrid,lincmargdist,linc_a,lincdelta,&
PERCinc(6),PERCinc(7),Ea_incQ(3))
	CALL ConditionalExpectation (naby,lincgrid,lincmargdist,linc_a,lincdelta,&
PERCinc(7),lincgrid(naby),Ea_incQ(4))

	CALL ConditionalExpectation (naby,lincgrid,lincmargdist,linc_b,lincdelta,&
lincgrid(1),PERCinc(5),Eb_incQ(1))
	CALL ConditionalExpectation (naby,lincgrid,lincmargdist,linc_b,lincdelta,&
PERCinc(5),PERCinc(6),Eb_incQ(2))
	CALL ConditionalExpectation (naby,lincgrid,lincmargdist,linc_b,lincdelta,&
PERCinc(6),PERCinc(7),Eb_incQ(3))
	CALL ConditionalExpectation (naby,lincgrid,lincmargdist,linc_b,lincdelta,&
PERCinc(7),lincgrid(naby),Eb_incQ(4))

	CALL ConditionalExpectation (naby,lincgrid,lincmargdist,linc_c,lincdelta,&
lincgrid(1),PERCinc(5),Ec_incQ(1))
	CALL ConditionalExpectation (naby,lincgrid,lincmargdist,linc_c,lincdelta,&
PERCinc(5),PERCinc(6),Ec_incQ(2))
	CALL ConditionalExpectation (naby,lincgrid,lincmargdist,linc_c,lincdelta,&
PERCinc(6),PERCinc(7),Ec_incQ(3))
	CALL ConditionalExpectation (naby,lincgrid,lincmargdist,linc_c,lincdelta,&
PERCinc(7),lincgrid(naby),Ec_incQ(4))

	CALL ConditionalExpectation (naby,lincgrid,lincmargdist,lincgrid,lincdelta,&
lincgrid(1),PERCinc(5),Einc_incQ(1))
	CALL ConditionalExpectation (naby,lincgrid,lincmargdist,lincgrid,lincdelta,&
PERCinc(5),PERCinc(6),Einc_incQ(2))
	CALL ConditionalExpectation (naby,lincgrid,lincmargdist,lincgrid,lincdelta,&
PERCinc(6),PERCinc(7),Einc_incQ(3))
	CALL ConditionalExpectation (naby,lincgrid,lincmargdist,lincgrid,lincdelta,&
PERCinc(7),lincgrid(naby),Einc_incQ(4))
