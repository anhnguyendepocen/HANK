...
(irfstruct(0)%equmSTICKY%rb - equmTRANS%rb)*equmINITSS%govbond
...
(irfstruct(0)%equmSTICKY%rb - equmTRANS%rb)*equmINITSS%govbond
				...
				CASE(12) !change ra, and change rb, rborr by same amount as ra
					equmTRANS%ra = irfstruct(0)%equmSTICKY%ra
					equmTRANS%rb = irfstruct(0)%equmSTICKY%rb + &
(irfstruct(0)%equmSTICKY%ra - equmINITSS%ra)
					equmTRANS%rborr = irfstruct(0)%equmSTICKY%rborr + &
(irfstruct(0)%equmSTICKY%ra - equmINITSS%ra)
				...
				CASE(14) !change rb, rborr, and change ra by same amount as rb, and discount eqm profits at implied ra
					...
					ELSE IF(DividendFundLumpSum==1) THEN
						it = Ttransition
						lequity = (equmFINALSS%equity + irfstruct(0)%equmSTICKY(it)%&
dividend*deltatransvec(it)) / (1.0+deltatransvec(it)*equmTRANS(it)%ra)		
						DO it = Ttransition-1,1,-1
							lequity = (lequity + irfstruct(0)%equmSTICKY(it)%&
dividend*deltatransvec(it)) / (1.0+deltatransvec(it)*equmTRANS(it)%ra)		
						END DO
						equmTRANS(:)%illassetdrop = ((1.0-irfstruct(0)%equmSTICKY(1)%&
fundlev) * irfstruct(0)%equmSTICKY(1)%capital + lequity) / ((1.0-equmINITSS%fundlev)*equmINITSS%capital&
 + equmINITSS%equity)
					END IF					
				...
				CASE(15) !change rb, rborr, and change ra by same amount as rb, and discount initimake al profits at implied ra
					equmTRANS%rb = irfstruct(0)%equmSTICKY%rb
					equmTRANS%rborr = irfstruct(0)%equmSTICKY%rborr
					equmTRANS%ra = equmINITSS%ra + (irfstruct(0)%equmSTICKY%rb - &
equmINITSS%rb)
					IF(DividendFundLumpSum==0) THEN
						equmTRANS(:)%illassetdrop = 1.0	
					ELSE IF(DividendFundLumpSum==1) THEN
						it = Ttransition
						lequity = (equmFINALSS%equity + &
equmINITSS%dividend*deltatransvec(it)) / (1.0+deltatransvec(it)*equmTRANS(it)%ra)		
						DO it = Ttransition-1,1,-1
							lequity = (lequity + &
equmINITSS%dividend*deltatransvec(it)) / (1.0+deltatransvec(it)*equmTRANS(it)%ra)		
						END DO
						equmTRANS(:)%illassetdrop = &
((1.0-irfstruct(0)%equmSTICKY(1)%fundlev) * irfstruct(0)%equmSTICKY(1)%capital + &
lequity) / ((1.0-equmINITSS%fundlev)*equmINITSS%capital + equmINITSS%equity)
					END IF					
