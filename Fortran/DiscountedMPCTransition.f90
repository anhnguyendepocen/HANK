...
REAL(8), DIMENSION(nab,ngpy) 	:: lsubeff1ass,lsubeff1ass1,lsubeff2ass,lsubeff2ass1,&
lwealtheff1ass,lwealtheff1ass1,lwealtheff2ass,lwealtheff2ass1
REAL(8), DIMENSION(nab,ngpy) 	:: lmpcvec,lmpdavec,lmpdbvec,lmargadjcostvec,&
lbadjvec,leffdiscvec,lmucvec
...
DO it = Ttransition,1,-1
	...
	DO iaby = 1,naby
		...
		leffdiscvec(abfromab(ia,ib),iy) = lmpcvec(abfromab(ia,ib),iy) + (1.0 + &
lmargadjcostvec(abfromab(ia,ib),iy))*lmpdbvec(abfromab(ia,ib),iy) - lmpdavec(abfromab(ia,ib),iy)
		lmucvec(abfromab(ia,ib),iy) = utilfn1(c(ia,ib,iy));
		lbadjvec(abfromab(ia,ib),iy) = bgrid(ib) * (mpc(ia,ib,iy)/c(ia,ib,iy)) *&
 lmucvec(abfromab(ia,ib),iy)
	...
	DO iy = 1,ngpy
		lvec = delta*lmucvec(:,iy)*ldrb + lsubeff1ass1(:,iy) + &
delta*MATMUL(ymarkovoff(iy,:),TRANSPOSE(lsubeff1ass1(:,:)))
	...
	!$OMP PARALLEL DO PRIVATE(lvec)
	DO iy = 1,ngpy
		lvec = delta*lmucvec(:,iy)*ldrb + lsubeff2ass1(:,iy) + &
delta*MATMUL(ymarkovoff(iy,:),TRANSPOSE(lsubeff2ass1(:,:)))
	...
	!$OMP PARALLEL DO PRIVATE(lvec)
	DO iy = 1,ngpy
		lvec = delta*lbadjvec(:,iy)*ldrb + lwealtheff1ass1(:,iy) + &
delta*MATMUL(ymarkovoff(iy,:),TRANSPOSE(lwealtheff1ass1(:,:)))
	...
	!$OMP PARALLEL DO PRIVATE(lvec)
	DO iy = 1,ngpy
		lvec = delta*lbadjvec(:,iy)*ldrb + lwealtheff2ass1(:,iy) + &
delta*MATMUL(ymarkovoff(iy,:),TRANSPOSE(lwealtheff2ass1(:,:)))
	...
	!$OMP PARALLEL DO
	DO iaby = 1,naby
		subeff1ass(afromaby(iaby),bfromaby(iaby),yfromaby(iaby)) = &
lsubeff1ass(abfromab(afromaby(iaby),bfromaby(iaby)),yfromaby(iaby))
		subeff2ass(afromaby(iaby),bfromaby(iaby),yfromaby(iaby)) = &
lsubeff2ass(abfromab(afromaby(iaby),bfromaby(iaby)),yfromaby(iaby))
		wealtheff1ass(afromaby(iaby),bfromaby(iaby),yfromaby(iaby)) = &
lwealtheff1ass(abfromab(afromaby(iaby),bfromaby(iaby)),yfromaby(iaby))
		wealtheff2ass(afromaby(iaby),bfromaby(iaby),yfromaby(iaby)) = &
lwealtheff2ass(abfromab(afromaby(iaby),bfromaby(iaby)),yfromaby(iaby))
	END DO
	!$OMP END PARALLEL DO
