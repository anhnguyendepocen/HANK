...
IF(DividendFundLumpSum==1) THEN
	IF(DistributeProfitsInProportion==0) capital = Ea/(1.0-fundlev + &
(1.0-mc)*(1.0-corptax)/(ra*KYratio))
	IF(DistributeProfitsInProportion==1) capital = Ea/(1.0-fundlev + &
(1.0-mc)*(1.0-corptax)*profdistfrac/(ra*KYratio))
END IF
...
IF(Display>=1) write(*,"(A,I2,A,E11.4,A,E11.4)") '  Rho iter ',neqmiter, &
', rho ',rho, ', K/N err',lKNratio/KNratio - 1.0

FnDiscountRate = lKNratio/KNratio - 1.0


IF(CalibrateDiscountRate==1) OPEN(3, FILE = trim(OutputDir) // &
'DiscountRateCalibration.txt', ACCESS = 'append')
IF(CalibrateRhoAtInitialGuess==1) OPEN(3, FILE = trim(OutputDir) // &
'DiscountRateAtInitialGuess.txt', ACCESS = 'append')
WRITE(3,*) '*******************************************'
...
caputil 	= 1.0
tfpadj = ((tfp**(1.0+utilelast)) * (mc*alpha/rcapital)**(alpha*utilelast))**&
(1.0/utilelastalpha)
taxrev = labtax*wage*labor - lumptransfer + corptax*profit
IF(DistributeProfitsInProportion == 1 .and. TaxHHProfitIncome == 1) taxrev = &
taxrev + labtax*(1.0-profdistfrac)*profit*(1.0-corptax)
